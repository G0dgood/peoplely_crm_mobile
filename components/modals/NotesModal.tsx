...
export default function NotesModal({ visible, onClose }: NotesModalProps) {
  ...
  const saveTimeoutRefs = useRef<{ [key: string]: ReturnType<typeof setTimeout> | null }>({});
  ...
  // Reload notes when modal becomes visible
  useEffect(() => {
    if (visible) {
      loadNotes();
    }
